...
class DatePickerCalendarState extends State<DatePickerCalendar> {
      ...
      return Row(
        ...
        children: [
          GestureDetector(
            onTap: () {
              if(_pickedDate != null){
                print("_pickedDate _pickedDate   _pickedDate ");
                BlocProvider.of<SetDateBloc>(context)
                    .add(AddNextDate(date: _pickedDate!));
              }else{
                String dateWithT = state.date;
                DateTime start = DateFormat("yyyy/MM/dd").parse(dateWithT);
                print("dateWithT dateWithT    "+start.toString());
                // DateTime dateTime = DateTime.parse(dateWithT);
                BlocProvider.of<SetDateBloc>(context)
                    .add(AddNextDate(date: start));
                BlocProvider.of<SetDateBloc>(context)
                    .add(ReadDateEvent());
              }
  ...
  String convertDateTimeWithPlus(DateTime now){
    ...
    Gregorian g = Gregorian(now.year, now.month, now.day-counter++);
